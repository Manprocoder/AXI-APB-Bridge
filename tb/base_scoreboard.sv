...
class base_scoreboard extends uvm_scoreboard;
   ...
   //Data members
    //
    protected axi_req_item raw_req;
    ...
    protected axi_req_item rd_req_q [$];
    protected axi_req_item wr_req_q [$];
    ...
    protected string sim_result_path;
    protected int sim_summary_file;
    ...
    //methods
extern virtual function void flush_req_queue();
...
extern virtual function void display_checker();
...
//
function void base_scoreboard::flush_req_queue();
	rd_req_q.delete();
	wr_req_q.delete();
endfunction
//
function void base_scoreboard::flush_data();
	axi_wdata_q.delete();
	axi_rdata_arr.delete();
endfunction
//
function void base_scoreboard::flush_resp();
    actual_bresp_array.delete();
    expected_bresp_array.delete();
endfunction
//
function void base_scoreboard::flush_sim_result();
	sim_result_arr.delete();
endfunction
//
//queue to store base and end address of supported APB SLAVES
function void base_scoreboard::init_slave_addr(input int no_addr, input logic [31:0] start_addr, input logic [31:0] page_size);
    logic [31:0] base_addr, end_addr;
    //
    begin
        for (int i = 0; i < no_addr; i++) begin
            base_addr = start_addr * (i+1);//32'h0000_0000 * (i+1);
...
//
function void base_scoreboard::flush_axi_addr_q;
	axi_addr_q.delete();
endfunction
//
function void base_scoreboard::flush_slave_addr;
	apb_base_end_addr_q.delete();
endfunction
//
//
function void base_scoreboard::init_attribute;
begin
	match = 0;
	mismatch = 0;
	last_beat = 1'b0;
...
//
function void base_scoreboard::init_checker();
	total_pass = 0;
	total_fail = 0;
endfunction
//
function void base_scoreboard::display_checker();
	`uvm_info(get_type_name, $sformatf("TOTAL PASS: %0d___TOTAL FAIL: %0d", total_pass, total_fail), UVM_LOW)
endfunction
//
//---------------------------------AXI_ADDR_CALCULATION FUNCTION---------------------
//
function void base_scoreboard::calculate_and_store_addr();
...
//
function logic [31:0] base_scoreboard::calculate_next_addr(input int i);
    ...
    //
    //
function void base_scoreboard::store_simulation_result(input resp_name status);
...
//
function void base_scoreboard::display_sim_res();
	...
	//open file
		...
		if(sim_summary_file == 0) begin
			`uvm_warning(get_type_name(), $sformatf("Failed to open summary.log"))
		end
        `endif
	if (sim_result_arr.size() > 0) begin
		...
		`ifdef PRINT_TO_SUM_FILE
		   $fdisplay(sim_summary_file, $sformatf("SIM RESULT of %0d(ID):", arr_idx));
		`else
...
//
function void base_scoreboard::Store_Expected_Bresp(input resp_name rsp);
//
...
//
function void base_scoreboard::display_rsp_report();
    ...
    //
    `ifdef PRINT_TO_SUM_FILE
    $fdisplay(sim_summary_file, $sformatf("WRITE_RSP CHECK: %0d(PASS)___%0d(FAIL)", pass_cnt, fail_cnt));
...
//
    ...
    //uvm_analysis_imp_AxiRdRequest #(axi_req_item, scoreboard) aimp_AxiRdRequest;
    //uvm_analysis_imp_AxiRData #(axi_data_item, scoreboard) aimp_AxiRData;
    //uvm_analysis_imp_AxiWrRequest #(axi_req_item, scoreboard) aimp_AxiWrRequest;
    //uvm_analysis_imp_AxiWData #(axi_data_item, scoreboard) aimp_AxiWData;
    //uvm_analysis_imp_AxiBresp #(axi_brsp_item, scoreboard) aimp_AxiBresp;
    ...
    //uvm_analysis_imp_ApbContent #(apb_item, scoreboard) aimp_ApbContent;
    ////--------------------------------------
    ////data members
    ////---------------------------------------
    ////
...
//endclass
    ...
    ////(1)
    //function void scoreboard::write_Aresetn(logic arst_n);
        //if (~arst_n) begin
            //axi_rst_flg = 1'b1;
            //flush_sim_result();
	    //flush_req_queue();
    ...
    ////(2)
    ////
    //function void scoreboard::write_AxiRdRequest(axi_req_item rd_req);
	//rd_req_q.push_back(rd_req);
    //endfunction
    ////(3)
    ////put rdata_item into associative array with queue element
    ////
    //function void scoreboard::write_AxiRData(axi_data_item rd_content);
	////    rd_content.print();
	    //data_q axi_rdata_q = {};
    ...
    //endfunction
    ////(4)
    //function void scoreboard::write_AxiWrRequest(axi_req_item wr_req);
		//wr_req_q.push_back(wr_req);
    //endfunction
    ////(5)
    //function void scoreboard::write_AxiWData(axi_data_item wr_content);
        //axi_wdata_q.push_back(wr_content);
    //endfunction
    ////(6)
    //function void scoreboard::write_AxiBresp(axi_brsp_item B_channel);
    ...
    ////----APB Protocol
    ////(1)
    //function void scoreboard::write_Presetn(logic preset_n);
        //if(~preset_n) begin
		   //apb_rst_flag = 1'b1;
		   //clr_member();
    ...
    ////
    ////(2)
    //function void scoreboard::write_ApbContent(apb_item ApbContent);
        //apb_content_q.push_back(ApbContent);
    //endfunction
    ////
    ////(3)
    //function void scoreboard::write_Pseltb(logic [`SLAVE_CNT-1:0] psel_tb);
    ...
    ////-------------------------------------------------------------------------------
//function void scoreboard::convert_axi_to_compare(axi_data_item axi_item);
...
////
////
//function void scoreboard::convert_apb_to_compare(apb_item apb_trans);
...
////
////
//function void scoreboard::clr_member();
    //done = 0;
    //sim_counter = 0;
    //cur_rsp = OKAY;
//endfunction
////
//function void scoreboard::flush_apb_queue();
    //apb_content_q.delete();
//endfunction
    ////
//task scoreboard::wait_apb_transfer(input int timeout);
////
//compare_start = 0;
//wait(apb_rst_flag == 0);
//fork
...
////
////
//task scoreboard::compare_transaction(
	 //input bit wr_en
//);
...
//task scoreboard:: Fetch_Valid_Req(input bit wr_en);
    ////
    //bit queue_not_empty;
    //bit error_case;
    ////
...
//endtask
    ////
    //function bit scoreboard::Check_Valid_Addr(input logic [31:0] start_address);
         //if(start_address <= apb_base_end_addr_q[`SLAVE_CNT*2-1]) return 1'b1;
	 //else return 1'b0;
    //endfunction
    ////
    //task scoreboard::Pop_unvalid_data();
        ////
	//bit wr_en;
	//bit data_rdy = 0;
...
////
////
//task scoreboard::Do_Comparison(input bit wr_en);
        ////
...
////compare task
////
//task scoreboard::compare_transfer(input int i);
    //string header;
...
////
////
//task scoreboard::handle_wait_data(input bit write_enable, input bit axi_or_apb, output bit break_enable);
    ////
    //resp_name wait_resp;
    //string header;
    ////
...
////
////
//function void scoreboard::check_missing_transaction;
//begin
	//while(1)begin
