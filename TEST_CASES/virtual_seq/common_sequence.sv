...
typedef axi_transaction#(DW,AW1) axi_item;
//
...
class AxiMasterWriteSeq #(DW, AW1) extends stimulus_generator;
...
//--------------AXI MASTER READ-------------------------
//
class AxiMasterReadSeq#(DW,AW1) extends stimulus_generator;
...
class rsp_seq extends stimulus_generator;
	...
	//
 	function void gen_item();
		mb = new(no_test_slverr + no_test_decerr);
		//mb = new();
		repeat(no_test_slverr) begin
			trans_h = axi_item::type_id::create("axi_item");
	...
	//
	virtual task body();
		repeat(no_test_slverr + no_test_decerr) begin
