...
//--File: addr_wdata_parallel_test.sv
//--Description: 
...
import env_pkg::*;
import seq_pkg::*;
class addr_wdata_parallel_test extends base_test;
    //register factory
    `uvm_component_utils(addr_wdata_parallel_test)
    //
  //  interface axi_intf#(DW1, AW1) axi_if_h; //=> without virtual keyword, compiler signals error
    ...
    //
    //constructor
    function new(string name = "addr_wdata_parallel_test", uvm_component parent = null);
...
//========================================================================
function void addr_wdata_parallel_test::build_phase(uvm_phase phase);
    ...
    //
    set_inst_override_by_type("env_h.axi_mst_agt.*", AxiMasterDriver::get_type(), axi_addr_wdata_parallel_driver::get_type());
    f_h.print();
    //
    ...
    //--only for trial
    uvm_config_db#(string)::set(this, "env_h.*", "TRIAL", "addr_wdata_parallel_test"); //absolute path
   // uvm_config_db#(string)::set(this, "axi_mst_agt", "TRIAL", "addr_wdata_parallel_test"); //relative path 
    //
...
//
//
function void addr_wdata_parallel_test::end_of_elaboration_phase(uvm_phase phase);
    `uvm_info(get_name(), "[END_OF_ELABORATION_PHASE UVM_TEST]", UVM_LOW)
    super.end_of_elaboration_phase(phase);
endfunction
//
function void addr_wdata_parallel_test::start_of_simulation_phase(uvm_phase phase);
    super.start_of_simulation_phase(phase);
    `uvm_info(get_name(), "[START_OF_SIMULATION_PHASE UVM_TEST]", UVM_LOW)
endfunction
//
//
task addr_wdata_parallel_test::run_phase(uvm_phase phase);
...
endtask
//
function void addr_wdata_parallel_test::check_phase(uvm_phase phase);
    `uvm_info(get_name(), "[CHECK_PHASE]", UVM_LOW);
endfunction
//
function void addr_wdata_parallel_test::report_phase(uvm_phase phase);
    ...
    //+ vseq.no_rdata_almost_full;
        `uvm_info(get_name(), "==================================================", UVM_LOW);
        `uvm_info(get_name(), "===========ADDR_DATA_PARALLE TEST REPORT==========", UVM_LOW);
        `uvm_info(get_name(), "==================================================", UVM_LOW);
        `uvm_info(get_name(), $sformatf("TRANSACTION IN TOTAL: %0d", total_trans), UVM_LOW)
endfunction
//
function void addr_wdata_parallel_test::final_phase(uvm_phase phase);
    `uvm_info(get_name(), "[FINAL_PHASE]", UVM_LOW);
endfunction
//
task addr_wdata_parallel_test::apply_reset();
