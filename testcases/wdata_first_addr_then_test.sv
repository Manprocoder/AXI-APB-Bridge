...
//--File: wdata_first_addr_then_test.sv
//--Description: 
...
import env_pkg::*;
import seq_pkg::*;
class wdata_first_addr_then_test extends base_test;
    //register factory
    `uvm_component_utils(wdata_first_addr_then_test)
    //
  //  interface axi_intf#(DW1, AW1) axi_if_h; //=> without virtual keyword, compiler signals error
    ...
    //
    //constructor
    function new(string name = "wdata_first_addr_then_test", uvm_component parent = null);
...
//========================================================================
function void wdata_first_addr_then_test::build_phase(uvm_phase phase);
    ...
    //
    set_inst_override_by_type("env_h.axi_mst_agt.*", AxiMasterDriver::get_type(), axi_wdata_first_addr_then_driver::get_type());
    f_h.print();
    //
    ...
    //--only for trial
    uvm_config_db#(string)::set(this, "env_h.*", "TRIAL", "wdata_first_addr_then_test"); //absolute path
   // uvm_config_db#(string)::set(this, "axi_mst_agt", "TRIAL", "wdata_first_addr_then_test"); //relative path 
    //
...
//
//
function void wdata_first_addr_then_test::end_of_elaboration_phase(uvm_phase phase);
    `uvm_info(get_name(), "[END_OF_ELABORATION_PHASE UVM_TEST]", UVM_LOW)
    super.end_of_elaboration_phase(phase);
endfunction
//
function void wdata_first_addr_then_test::start_of_simulation_phase(uvm_phase phase);
    super.start_of_simulation_phase(phase);
    `uvm_info(get_name(), "[START_OF_SIMULATION_PHASE UVM_TEST]", UVM_LOW)
endfunction
//
//
task wdata_first_addr_then_test::run_phase(uvm_phase phase);
...
endtask
//
function void wdata_first_addr_then_test::check_phase(uvm_phase phase);
    `uvm_info(get_name(), "[CHECK_PHASE]", UVM_LOW);
endfunction
//
function void wdata_first_addr_then_test::report_phase(uvm_phase phase);
int total_trans = vseq.no_rd_wr_para + vseq.no_rd_wr_rd + vseq.no_wr_rd_wr;// + vseq.no_rdata_almost_full; 
    ...
    //+ vseq.no_rdata_almost_full;
        `uvm_info(get_name(), "==================================================", UVM_LOW);
        `uvm_info(get_name(), "=========WDATA_FIRST_ADDR_THEN TEST REPORT========", UVM_LOW);
        `uvm_info(get_name(), "==================================================", UVM_LOW);
        `uvm_info(get_name(), $sformatf("TRANSACTION IN TOTAL: %0d", total_trans), UVM_LOW)
endfunction
//
function void wdata_first_addr_then_test::final_phase(uvm_phase phase);
    `uvm_info(get_name(), "[FINAL_PHASE]", UVM_LOW);
endfunction
//
task wdata_first_addr_then_test::apply_reset();
