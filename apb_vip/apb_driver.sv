...
parameter DEPTH = 4096; //using localparam causes error 
  ...
  logic [DW2-1:0] new_value, current_value;
  //
...
task apb_driver::reset_all();
  wait (~drv_cfg.vif.presetn) begin
	...
	drv_cfg.vif.s_drv_cb.pready <= {`SLAVE_CNT{1'b0}};
	drv_cfg.vif.s_drv_cb.pslverr <= {`SLAVE_CNT{1'b0}};
      end
    mem_init();
  end
...
foreach(drv_cfg.vif.s_drv_cb.psel[i]) begin
if(drv_cfg.vif.s_drv_cb.psel[i]) begin//PSEL
	if(drv_cfg.vif.s_drv_cb.pwrite && drv_cfg.vif.s_drv_cb.penable) begin
	mem_access(i, 1'b1, drv_cfg.vif.s_drv_cb.pstrb, drv_cfg.vif.s_drv_cb.paddr[11:0], drv_cfg.vif.s_drv_cb.pwdata, rdata);
	done2 = 1'b1;
	...
	mem_access(i, 1'b0, drv_cfg.vif.s_drv_cb.pstrb, drv_cfg.vif.s_drv_cb.paddr[11:0], drv_cfg.vif.s_drv_cb.pwdata, rdata);
	drv_cfg.vif.s_drv_cb.prdata[i] <= rdata;
	done2 = 1'b1;
...
if(drv_cfg.vif.s_drv_cb.psel[i]) begin //PSEL
	...
	// usually 1 (ready)
	drv_cfg.vif.s_drv_cb.pslverr <= packet.pslverr; // usually 0 (no error)
	//
	if(~packet.pready[i] && drv_cfg.vif.s_drv_cb.penable) counter++;
end//end of PSEL
end//end of foreach
if(done) begin
break;
