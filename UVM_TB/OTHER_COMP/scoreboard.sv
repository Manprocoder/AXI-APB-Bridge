...
class base_scoreboard extends uvm_scoreboard;
    ...
    protected int axi_data_wait_cnt; //avoid waiting infinite loop with empty status of axi_rdata_arr OR axi_wdata_q
    ...
    protected data_q axi_rdata_arr [id_of_burst]; //each id_of_burst maps to an individual q
...
//
virtual function void init_checker();
	total_pass = 0;
	total_fail = 0;
endfunction
//
virtual function void display_checker();
...
//	header = (parsed_req.wr_or_rd == 1) ? "WRITE" : "READ";
    ...
    //
    //
virtual function void store_simulation_result(input resp_name actual_status);
...
//
virtual function void Store_Expected_Bresp(input resp_name rsp);
//
