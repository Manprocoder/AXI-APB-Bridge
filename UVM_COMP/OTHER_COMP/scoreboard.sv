...
//	string header;
	...
	//
    begin
//	header = (parsed_req.wr_or_rd == 1) ? "WRITE" : "READ";
        //
...
virtual function void store_simulation_result(
	...
	if(sim_result_arr.exists(raw_req.id[7:0])) begin
		...
		$sformatf("[EXISTING]Store SIM RESULT of [%s][%0d(ID)]",
	       	raw_req.wr_or_rd ? "WRITE" : "READ_", raw_req.id[7:0]), UVM_LOW)
	end
	else begin
		tmp_q.push_back(Transaction_result);
		sim_result_arr[raw_req.id[7:0]] = tmp_q;
...
//
virtual function void display_sim_res();
...
//
//
virtual task Store_Expected_Bresp(input id_of_burst id, input resp_name rsp);
//
...
endtask
//
virtual function void display_rsp_report();
...
extern virtual task wait_apb_transfer(input int timeout_consecutive_times);
...
extern virtual task Pop_unvalid_data(input bit wr_en);
...
endclass
	...
	//    rd_content.print();
	    data_q axi_rdata_q = {};
	if(axi_rdata_array.exists(rd_content.id)) begin
		axi_rdata_q = axi_rdata_array[rd_content.id];
		axi_rdata_q.push_back(rd_content);
		axi_rdata_array[rd_content.id] = axi_rdata_q;
		`uvm_info(get_type_name(), $sformatf("[EXIST]Push Rd_content into axi_rdata_array[%0d]", rd_content.id), UVM_MEDIUM);
	end
	else begin
		axi_rdata_q.push_back(rd_content);
		axi_rdata_array[rd_content.id] = axi_rdata_q;
		`uvm_info(get_type_name(), $sformatf("[NEW]Push Rd_content into axi_rdata_array[%0d]", rd_content.id), UVM_MEDIUM);
	end
        //
    endfunction
    //(4)
...
function void scoreboard::flush_apb_queue();
    ...
    //
task scoreboard::wait_apb_transfer(input int timeout_consecutive_times);
//
compare_start = 0;
wait(apb_rst_flag == 0);
fork
...
task scoreboard:: Fetch_Valid_Req(input bit wr_en);
    ...
    begin//begin_end block
		...
		error_case = (raw_req.size != 3'b010);//unsupported transactions
                //
                if(!Check_Valid_Addr(raw_req.addr) || (error_case)) begin
...
endtask
    ...
    //
    task scoreboard::Pop_unvalid_data(input bit wr_en);
        //
